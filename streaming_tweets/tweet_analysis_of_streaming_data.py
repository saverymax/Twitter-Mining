...
To call from command line: python tweet_analysis_of_streaming_data.py --dir liberal
"""
...
def get_parser():
    ...
    parser.add_argument("--dir", 
                        dest = "directory", 
                        help = "The directory containing stream files")
    return parser
...
class tweetflow():
    ...
    def __init__(self, directory, tokens_re, emoticon_re, excess_symbols):
    ...
    def read_tweets(self):
        ...
        tweet_dataframe.to_csv('data/{}/converted_tweets.tsv'.format(self.directory), sep='\t', index = False)
...
class visualize():
    ...
    def __init__(self, terms, term_type, directory):
    ...
    def visualize_term_usage(self):
        ...
        axis.set_title('{0} usage in tweets'.format(self.name), fontsize = 20)
        tweet_figure.savefig('data/{0}/frequencies_{1}.png'.format(self.directory, self.name), bbox_inches = 'tight')  
        plt.show()
        plt.close()
...
if __name__ == '__main__':
    ...
    # Don't need this any more but useful splitting function
    #filename, stream_name = convert_name(args.filename)
    
    tweet_processing = tweetflow(args.directory, tokens_re, emoticon_re, excess_symbols)
    tweets = tweet_processing.read_tweets()# Lookup file
    # Index 0 holds the words of the tweet; index 1 holds the dataframe.
    processed_tweets = tweet_processing.process_tweets(tweets)

    common_words = visualize(processed_tweets[0], "Term", args.directory)
    common_words.visualize_term_usage()
    common_hash = visualize(processed_tweets[1], "Hashtag", args.directory)
